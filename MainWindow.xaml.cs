...
namespace GreenCityManagement
{
    public partial class MainWindow : Window
    {
        ...
        private void ApplyRolePermissions()
        {
            ...
            BtnDeletePlant.IsEnabled = isAdmin || isManager;   
        ...
        private void ExportPlants_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                using var db = new GreenCityContext();
                var data = db.Plant
                    ...
                    .ToList();
                Helpers.ExcelExporter.ExportPlants(data);
            }
            catch (Exception ex)
            {
        ...
        private void ExportEmployees_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                Helpers.ExcelExporter.ExportEmployees(data);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Ошибка экспорта: {ex.Message}", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void ExportMaintenance_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                using var db = new GreenCityContext();
                var data = db.Maintenance
                    .Include(m => m.Plant)
                    .Include(m => m.WorkType)
                    .Include(m => m.Employee)
                    .ToList();
                Helpers.ExcelExporter.ExportMaintenance(data);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Ошибка экспорта: {ex.Message}", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
